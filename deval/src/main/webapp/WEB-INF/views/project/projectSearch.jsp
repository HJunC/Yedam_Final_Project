		...
		<div class="row">
			...
			<div class="col-md-4 mt-30 wow fadeInUpShort" data-wow-delay=".1s">
				<div class="mod-breadcrumbs text-end">
					<a href="/project/main.do">프로젝트</a>&nbsp;<span
						class="mod-breadcrumbs-slash">•</span>&nbsp;<span>프로젝트 검색</span>
				</div>
			</div>
	...
	<div class="container relative">
		...
		<div class="row">
					...
					<c:if test="${not empty projectList}">
						...
						<c:forEach items="${board}" var="item">
							<a href="projectDetail.do?no=${item.projectNo}"
								class="list-group-item d-flex justify-content-between align-items-start pt-3 pb-3"
								aria-current="true">
								<div class="ms-2 me-auto">
									<div class="countdown-search-list mb-2">
										<i class="fa fa-users"></i>
										(${item.crntRcnt}/${item.totalRcnt})&nbsp;&nbsp;&nbsp; <i
											class="fa fa-stopwatch"></i> 마감 <span
											id="timer${item.projectNo}"></span>
									</div>
									<div>
										<h5 class="mb-2">${item.projectName}
								...
								<div>
									<div>
										<span class="info-search-list">남은 포지션</span> <span
											class="badge me-1 ${item.frontRcnt > 0 ? 'bg-custom-primary' : 'bg-dark'}"
											data-bs-toggle="tooltip" data-bs-placement="bottom"
											title="프론트엔드 개발자">FE ${item.frontRcnt}</span> <span
											class="badge me-1 ${item.backRcnt > 0 ? 'bg-custom-primary' : 'bg-dark'}"
											data-bs-toggle="tooltip" data-bs-placement="bottom"
											title="백엔드 개발자">BE ${item.backRcnt}</span> <span
											class="badge me-1 ${item.fullRcnt > 0 ? 'bg-custom-primary' : 'bg-dark'}"
											data-bs-toggle="tooltip" data-bs-placement="bottom"
											title="풀스택 개발자">FS ${item.fullRcnt}</span> <span
											class="badge me-1 ${item.designRcnt > 0 ? 'bg-custom-primary' : 'bg-dark'}"
											data-bs-toggle="tooltip" data-bs-placement="bottom"
											title="디자이너">DE ${item.designRcnt}</span> <span
											class="badge ${item.plannerRcnt > 0 ? 'bg-custom-primary' : 'bg-dark'}"
											data-bs-toggle="tooltip" data-bs-placement="bottom"
											title="기획자">PL ${item.plannerRcnt}</span>
									</div>
									<div class="info-search-list mt-40">
										<i class="fa fa-eye"></i> ${item.hit}&nbsp;&nbsp;&nbsp; <i
											class="fa fa-user-clock"></i> ${item.applyRcnt} 지원</span>
									</div>
								</div>
							</a>
						</c:forEach>
			...
			<div class="col-md-4 col-lg-3 mt-10">
				...
				<form class="form" id="searchForm" action="../project/search.do"
					method="get">
					...
					<div class="widget">
						<h3 class="widget-title d-flex justify-content-between">
							프로젝트 명
							<button type="button" onclick="resetForm()" class="btn btn-dark">
								<i class="fa fa-sync"></i>
							</button>
						</h3>

						<div class="search-wrap">
							<button class="search-button animate" type="button"
								title="Start Search" onclick="search()">
								<i class="fa fa-search"></i>
							</button>
							<input type="text" class="form-control search-field round"
								name="projectName" placeholder="Search..."
					...
					<div class="widget">
						...
						<div class="widget-body">
							<div class="tags">
								<input type="checkbox" class="btn-check" id="FE"
									name="frontRcnt" value="1"
									${search.frontRcnt != 0 ? 'checked' : null}> <label
									class="btn btn-outline-primary" for="FE">프론트엔드</label> <input
									type="checkbox" class="btn-check" id="BE" name="backRcnt"
									value="1" ${search.backRcnt != 0 ? 'checked' : null}> <label
									class="btn btn-outline-primary" for="BE">백엔드</label> <input
									type="checkbox" class="btn-check" id="FS" name="fullRcnt"
									value="1" ${search.fullRcnt != 0 ? 'checked' : null}> <label
									class="btn btn-outline-primary" for="FS">풀스택</label> <input
									type="checkbox" class="btn-check" id="DE" name="designRcnt"
									value="1" ${search.designRcnt != 0 ? 'checked' : null}>
								<label class="btn btn-outline-primary" for="DE">디자이너</label> <input
									type="checkbox" class="btn-check" id="PL" name="plannerRcnt"
									value="1" ${search.plannerRcnt != 0 ? 'checked' : null}>
							...
							<div class="row">
								<div class="col">
									<div class="input-group">
										<input type="number" name="termStart"
											class="input-sm round form-control bg-dark-input" min="3"
											max="365"
											value="${search.termStart == null or search.termStart eq '' ? 3 : search.termStart}">
										<div class="input-group-text bg-dark"
											style="border-color: #5e646a;">일</div>
									</div>
								</div>
								~
								<div class="col">
									<div class="input-group">
										<input type="number" name="termEnd"
											class="input-sm round form-control bg-dark-input" min="3"
											max="365"
											value="${search.termEnd == null or search.termEnd eq ''  ? 365 : search.termEnd}">
										<div class="input-group-text bg-dark"
											style="border-color: #5e646a;">일</div>
									</div>
								</div>
					...
					<div class="widget">
						...
						<div class="widget-body">
							<div class="tags">
								<div class="tags">
									<input type="radio" class="btn-check" id="ON" name="process"
										value="ON" ${search.process eq 'ON' ? 'checked' : null}>
									<label class="btn btn-outline-primary" for="ON">온라인</label> <input
										type="radio" class="btn-check" id="OFF" name="process"
										value="OFF" ${search.process eq 'OFF' ? 'checked' : null}>
									<label class="btn btn-outline-primary" for="OFF">오프라인</label> <input
										type="radio" class="btn-check" id="ONOFF" name="process"
										value="ONOFF" ${search.process eq 'ONOFF' ? 'checked' : null}>
